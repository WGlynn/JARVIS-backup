...
**7/** Anti-hallucination chain. Substance gate = regex pattern matching with context disambiguators.

Flagged term must have correct context regexes nearby; forbidden regexes block the write.

Born from a real "clawback ≠ forfeiture" miss in a USD8 doc — caught at write-time before the term hit a Solidity selector.

**8/** Discipline layer. Patterns surface in real time. Captured at 3+ instances as primitives, before I name them.

Recent saves: `scope-drift-to-recent`, `structurally-easier-partner-delivery`, `draft-replies-on-behalf-of-others`.
...
→ Standalone signature validator (38 tests)
